...
| `NO_SECRET_IN_LOCALSTORAGE` | Nenhum segredo é gravado em `localStorage`. | Correção real (Diretriz Consolidação/Auditoria/Evolução, auditoria de performance/infra, achado confirmado por busca no filesystem): esta linha afirmava que `storage.js` implementava "OPFS com fallback IndexedDB" e que `localStorage` não era usado em nenhum arquivo — nenhuma das duas é verdade hoje. `storage.js` não existe em nenhum lugar do repositório. `localStorage` é usado em 3 lugares reais de `ramber-ui/src/` (`App.tsx`: `SESSION_STATE_KEY`/`WIDGET_PREFS_KEY`; `access-gate.tsx`: flag de unlock) — nenhum é segredo/credencial. A garantia real permanece: persistência de dado (candles, track record) usa IndexedDB de verdade via `nexus/persistence.ts` (pacote `idb`, DB `ar10-cyborg-nexus`), nunca `localStorage`; nenhum caminho de código grava segredo em nenhum dos dois. |
