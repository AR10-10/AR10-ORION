...
describe('App.tsx: resolvedTradFiInstrument liga selectedTradFiAsset ao Instrument Registry real', () => {
  ...
  it('todo o resto do modo TRADFI (order book/flow/heatmap/Siriform/Council/Regime) continua em TradFiEmptyState — Fase 1 não expande escopo além do gráfico principal', () => {
    const app = read('../src/App.tsx');
    const tradfiEmptyStateCount = (app.match(/<TradFiEmptyState/g) ?? []).length;
    // 1 no branch sem mapeamento do gráfico principal + 7 nos painéis
    // crypto-específicos (Market Intelligence, Siriform, Regime/Comitê,
    // Siriform detalhe, Order Book, Order Flow, Liquidity Map) — mesma
    // contagem de antes desta Ordem, só o primeiro ganhou um irmão real.
    expect(tradfiEmptyStateCount).toBe(8);
  });
});
