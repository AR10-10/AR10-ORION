...
describe('EnhancedChart_110_Percent.tsx: lista canônica única de camadas, todas visíveis por padrão', () => {
  ...
  it('DEFAULT_CHART_LAYER_VISIBILITY liga as 16 camadas por padrão — o painel nunca esconde nada sem ação explícita do Operador', () => {
    const chart = read('../src/chart/EnhancedChart_110_Percent.tsx');
    ...
    for (const key of ['liquidity_zones', 'structure_breaks', 'order_flow_heatmap', 'volume_profile', 'trade_plan_zone', 'neural_market_aura', 'ema', 'trend_channel', 'vwap', 'nexus_line', 'cvd', 'fibonacci', 'premium_discount', 'harmonics', 'equal_highs_lows', 'liquidation_heatmap']) {
...
describe('App.tsx: estado real do painel + toggle por camada, compartilhado via contextValue', () => {
  ...
  it('CHART_LAYER_PANEL_MODULES lista exatamente as 16 camadas reais, cada id um ChartLayerId válido (o próprio TypeScript trava isso — este teste só confirma que a lista não encolheu/cresceu silenciosamente)', () => {
    const app = read('../src/App.tsx');
